...
def get_list():
    sql = "SELECT *, (SELECT username FROM users WHERE D.user_id = id) AS username FROM discussions D ORDER BY D.created_at DESC"
    result = db.db.session.execute(db.text(sql))
    return result.fetchall()

def get_list_by_sub_id(id):
    sql = "SELECT *, (SELECT username FROM users WHERE D.user_id=id) AS username FROM discussions D WHERE D.subforum_id=:id ORDER BY D.created_at DESC"
